        ...
        if st.button("🎯 予測実行", type="primary", key="predict_button"):
            if not selected_player:
                ...
                if player_stats.empty:
                    ...
                    # タイトル獲得情報を表示
                    if player_stats['タイトル数'] > 0:
                        st.markdown("---")
                        st.info(f"🏆 {stats_year}年のタイトル獲得数: **{int(player_stats['タイトル数'])}個**")
...
# フッター
st.markdown("---")
st.markdown("*NPB選手年俸予測システム（対数変換版 + 減額制限対応 + 年齢考慮） - Powered by Streamlit*").warning(f"""
                            ⚖️ **減額制限に引っかかります**
                            - 前年年俸: {previous_salary/1e6:.1f}百万円
                            - 予測年俸: {predicted_salary/1e6:.1f}百万円
                            - 減額制限: {reduction_rate*100:.0f}%まで（最低{(1-reduction_rate)*100:.0f}%保証）
